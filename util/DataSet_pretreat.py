...
class MiniImageNetDataSet(data.Dataset):
    def __init__(self,root, mode="train_train",n_way = 5,mosaic = False,mosaic_novel_rate=1, mosaic_cls=3,Location=False,sample_idx=None,
        ...
        self.elapse_time_IO = 0
        self.elapse_time_transform = 0
        self.img_dict = {}
        self.img_base_dict = {}
        ...
        if self.mosaic_center:
            ...
            preload_base_img(self.dict_path,self.center_sample)
        # Base 预训练阶段
        if self.mode != "fsl":
            if mosaic:
                ...
                if self.Location:
                    if self.mosaic:
                        if self.is_BaseMosaic:
                            mosaic_img, label_loc = self.Base_Mosaic(img, idx, label)
                        else:
                            mosaic_img, label_loc = self.mosaicImg(img,idx,label)


                        return img_,mosaic_img,label,label_loc
    ...
    def fsl_samples(self,image_list,label_lsit,sample_idx_list,mosaic_k=1):
        samples = []
        label_dict= {}
        ind = 0
        ...
        for idx in sample_idx_list:
            ...
            if label not in label_dict.keys():
                ...
                ind += 1
            item = img_path,label_dict[label]
            if mosaic_k == 1:
                img = Image.open(img_path).convert('RGB')
    ...
    def Base_Mosaic(self,img,idx,label,origin=False):
        ...
        mosaic_idx_list = self.mosaic_sample(label,self.grid)

        for idx in mosaic_idx_list:
            r, c = int(idxs[i] / self.grid), int(idxs[i] % self.grid)
            if idx == label:
                label_loc.data[idxs[i]] = 1
            sample_list = self.mosaic_nk[idx][:]
            mosaic_idx = np.random.choice(sample_list,1)
            img_idx = np.random.choice(self.center_sample[mosaic_idx, :])
            img_path = self.dict_path[img_idx]
            img_m = self.img_base_dict[img_path]
            if self.transform_mosaic:
                self.transform_mosaic.__dict__['transforms'][1] = T.RandomCrop(
                    (int(r_list[r + 1] - r_list[r]), int(c_list[c + 1] - c_list[c])))
                img_m = self.transform_mosaic(img_m)
    ...
    # 采样出 mosaic img中的类分布
    def mosaic_sample(self,label,grid):
        sample_list = np.where(np.arange(self.n_way) != label)[0]
        mosaic_list = random.sample(list(sample_list),grid**2-1)
        mosaic_list.append(label)
        return mosaic_list
    ...
    def preload_base_img(self,dict_path,center_sample):
        for i in range(center_sample.shape[0]):
            for j in range(center_sample.shape[1]):
                img_idx = center_sample[i,j]
                img_path = dict_path[img_idx]
                img = Image.open(img_path).convert('RGB')
                self.img_base_dict[img_path] = img
