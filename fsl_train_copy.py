...
def train():
    ...
    backbone = filter(lambda x:id(x) not in list(map(id,net.fc.parameters())) +list(map(id,net.fc_loc.parameters())),net.parameters())
    head = filter(lambda x:id(x)  in list(map(id,net.fc.parameters())) +list(map(id,net.fc_loc.parameters())),net.parameters())

    head_param = [{'params':head,'lr':args.lr_base}]
    backbone_param =  [{'params':backbone, 'lr':config.train.lr_backbone}]
    ...
    if config.train.optimizer == "Adam":
        optimizer = torch.optim.Adam(head, weight_decay=float(config.train.weight_decay))
        optimizer_back = torch.optim.Adam(backbone_param, lr=config.train.lr_backbone,weight_decay=float(config.train.weight_decay))
    elif config.train.optimizer == "SGD":
        optimizer = torch.optim.SGD(head_param,lr=args.lr_base ,momentum=config.train.momentum,
                                    weight_decay=float(config.train.weight_decay))
        optimizer_back = torch.optim.SGD(backbone_param, lr=config.train.lr_backbone, momentum=config.train.momentum,
                                    weight_decay=float(config.train.weight_decay))
    ...
    for epoch in range(Epoch_num):
        ...
        for step,(image,mosaic_img,label,lable_loc) in enumerate(trainloader):
            ...
            if args.proto_loss:
                ...
                feat ,out = net(image,is_feat=True)
                _,out_loc = net(mosaic_img,feat,is_loc= True)
                ___, prototypes_update = prototypical_Loss(feat, label, prototypes, epoch,
                ...
                if args.Few_loss:
                    ...
                    # ------------在CE loss 中添加 proto loss------------------
                # loss_cls_proto, prototypes_update = prototypical_Loss(feat, label, prototypes, epoch,
                #                                                       temperature=config.train.temperature)
                # prototypes = prototypes_update.to(device)
                # loss_cls = loss_cls + 0.1 * loss_cls_proto
            if args.boundingbox:
        ...
        correct = total = 0
        with torch.no_grad():
            if Oringin_loader != None:
                global devloader
                for step, (img, label) in enumerate(Oringin_loader):
                    img = img.to(device)
                    label = label.to(device)
                    feat_predict, _ = net(img, is_feat=True)
                    dist = euclidean_dist(feat_predict, prototypes)
                    ind = torch.min(dist,dim = 0)[1]
                    # print(ind)
                    sup = numpy.array(support_list)
                    # print(sup[ind.cpu().int().numpy()])
                    center_support_list = sup[ind.cpu().int().numpy()]
                    devloader = FSL_sample(only_test = True,center_list=list(center_support_list),fsl_cls=fsl_cls,query_list=query_list)

                # devloader = devloader_new
            for step, (img, label) in enumerate(devloader):
                ...
                if args.proto_loss:
                    ...
                    predict= net(img)
                if args.test_mosaic:
                    res = predict.reshape(-1,test_mosaic,args.n_way,predict.shape[1])
                    res = torch.mean(res,dim=1)
                    predict = torch.max(res,dim=1)[0]
                    label = label.reshape(-1,test_mosaic*args.n_way)
                    label = torch.max(label.float(),dim=1)[0]
...
def FSL_sample(only_test = False,center_list = None,fsl_cls = [],query_list=[]):
    ...
    DataSet_mosaic = MiniImageNetDataSet(root=config.data.root, mode=fsl_stage+"_fsl",n_way=args.n_way, sample_idx=support_list,
                                        ...
                                        mosaic_origin_num=args.mosaic_ori_num, boundingbox=args.boundingbox,size_bias=args.size_bias,
                                         grid=args.grid,fsl_label=fsl_cls,random_mosaic=args.random_mosaic,is_BaseMosaic=args.base_mos)

    mosaic_loader = torch.utils.data.DataLoader(DataSet_mosaic, batch_size=config.data.testloader.batch_size, shuffle=True,
                                            num_workers=config.data.testloader.worker)

    if args.test_mosaic == True:
        DataSet_val = MiniImageNetDataSet(root=config.data.root, mode=fsl_stage+"_fsl",n_way=args.n_way, sample_idx=query_list,
                                        test_mosaic_ob=support_list, mosaic_k=test_mosaic,transform=transform,
                                        mosaic_origin_num=args.mosaic_ori_num,grid=args.grid,fsl_label=fsl_cls)

        DataSet_origin = MiniImageNetDataSet(root=config.data.root, mode=fsl_stage + "_fsl", sample_idx=support_list,
                                          transform=transform)
        Oringin_loader = torch.utils.data.DataLoader(DataSet_origin, batch_size=args.n_way*args.k_shot,
                                            num_workers=config.data.testloader.worker)
    else:
    ...
    devloader = torch.utils.data.DataLoader(DataSet_val, batch_size=config.data.testloader.batch_size,
                                            num_workers=config.data.testloader.worker)
    return mosaic_loader,devloader,Oringin_loader,support_list,query_list,fsl_cls
...
if __name__ == '__main__':
    ...
    for i in range(args.num_task):
        ...
        # 根据小样本类构造相似类:
        # 抽样小样本类
        trainloader,devloader,Oringin_loader,support_list,query_list,fsl_cls = FSL_sample()
