...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, num_classes=-1, use_se=False, Location=False,grid=2,baseline=False):
        ...
        if self.Location:
            self.fc_loc = nn.Linear(640*2, grid**2)
        if self.num_classes > 0:
            if baseline:
                self.fc = nn.Linear(640, self.num_classes,bias=False)
            else:
    ...
    def forward(self, x, prompt=None,is_feat=False,is_loc=False):
        x = self.layer1(x)
        f0 = x
        x = self.layer2(x)
        f1 = x
        ...
        if self.num_classes > 0:
            if self.baseline:
                ...
                # self.fc.weight.data = self.fc.weight.data.div(fc_norm+1e-5)
                feat = x
            x = self.fc(x)

        if self.Location and is_loc:
            # ------------------6/21 11：00 修改-------------------------
            # 考虑梯度不回传的情况
            feat = feat.detach()
            feat_loc = torch.cat([feat,prompt],1)
            # print(feat_loc.size())
            if is_feat:
                return feat, x,self.fc_loc(feat_loc)
            else:
                return x,self.fc_loc(feat_loc)
        else:
            if is_feat:
                return feat, x
            else:
...
def resnet12(keep_prob=1.0, avg_pool=True, Location=False, **kwargs):
    """Constructs a ResNet-12 model.
    """
    model = ResNet(Bottleneck, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, Location=Location, **kwargs)
    return model
